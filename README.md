...
1. 将一切行动（即使是思考和说话）都作为工具调用的行为要求，且支持流式工具调用（暂时没有开关）
2. 自动收集和衰减的记忆系统（还需进化）
3. 自我调用（要解耦一下）
4. 允许存在三种不同的时间，分为真实世界时间，agent世界时间，agent主观时长，主观时长主要用于记忆
5. agent可能暂时不活跃，回复不了你的消息（还需配合之后的设定更新进一步完善）
6. 可以中途打断agent（暂时没有开关），它能知道自己被打断了
7. 嗯，有一个非常难用的配置系统
...
2. (现实世界 - 现实时间锚点) * 时间膨胀 + agent（世界/主观）时间锚点 = agent（世界/主观）时间
...
检查`config/sprite_comments.toml`以了解自定义配置，可在`sprites.toml`中设置自定义sprite配置
...
- [langgraph](https://github.com/langchain-ai/langgraph)，用于给原本的`add_messages`加点功能，虽说目前好像不需要了。还有binop的部分代码，我需要在图外也能合并state。
