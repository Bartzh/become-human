...
class MemoryConfig(StoreModel):
    ...
    memory_max_ttl: int = StoreField(default=30 * 365 * 24 * 60 * 60 * 1_000_000, title='记忆最大时长', description="记忆最大时长，单位为秒")
    ...
    exclude_low_similarity: bool = StoreField(default=True, title='排除低相似度记忆', description="是否排除低相似度的记忆")
