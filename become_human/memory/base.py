...
class MemoryManager():
    ...
    async def tick_all_memories(self, sprite_id: str) -> None:
        ...
        for t in store_manager.get_model(sprite_id, MemoryConfig).memory_types:
            update_count += await self.tick_memories(sprite_id, t)

        logger.debug(f'updated {update_count} memories for sprite "{sprite_id}".')

    async def tick_memories(
        self,
        sprite_id: str,
        memory_type: str,
        ...
        if ids_to_delete:
            await self.delete(sprite_id, memory_type, ids=list(ids_to_delete))
        await self.update_metadatas(ids_new, metadatas_new, memory_type, sprite_id)
    ...
    async def add_memories(self, memories: list[InitialMemory], sprite_id: str) -> None:
        ...
        for t in docs.keys():
            if docs[t]:
                await self.add_documents(docs[t], sprite_id, t)
    ...
    async def retrieve_memories(
        ...
        if (creation_time_range_start or creation_time_range_end) and not search_string:
            ...
            for retrieval in retrieval_config.retrievals:
                ...
                await self.update_metadatas(ids, metadatas, retrieval.memory_type, sprite_id)
                if first_memory:
            ...
            for memory_type, delete_ids in ids_to_delete.items():
                if delete_ids:
                    await self.delete(sprite_id, memory_type, ids=list(delete_ids))
            ...
            for retrieval in retrieval_config.retrievals:
                if retrieval.retrieve_method == "similarity":
                    docs_and_scores = await self.similarity_search_by_vector_with_score_and_retrievability(
                    ...
                    docs_and_scores = await self.max_marginal_relevance_search_by_vector_with_retrievability(
                        embedding=search_embedding,
                        sprite_id=sprite_id,
                        memory_type=retrieval.memory_type,
                        k=retrieval.k,
    ...
    async def get_memories(
        ...
        get_result = await self.get(sprite_id, memory_type, ids, where, limit, offset, where_document)
        ...
        for index, metadata in enumerate(get_result['metadatas']):
            ...
            if patch.get('forgot'):
                ...
                update_ids.append(get_result['ids'][index])
                update_metas.append(patch)
        if remove_ids:
            await self.delete(sprite_id, memory_type, ids=remove_ids)
        if update_ids:
            await self.update_metadatas(update_ids, update_metas, memory_type, sprite_id)
        docs = [doc for idx, doc in enumerate(docs) if idx not in remove_idx]
        for i, meta in enumerate(update_metas):
            docs[i].metadata.update(meta)
        return docs



    async def update_metadatas(self, ids: list[str], metadatas: list[dict], memory_type: str, sprite_id: str) -> None:
        def _update_metadatas(ids: list[str], metadatas: list[dict], memory_type: str, sprite_id: str) -> None:
            if not ids:
                return
            if len(ids) != len(metadatas):
                raise ValueError("ids and metadatas must have the same length")
            self.get_collection(sprite_id, memory_type).update(ids=ids, metadatas=metadatas)
        await run_in_executor(None, _update_metadatas, ids, metadatas, memory_type, sprite_id)
    ...
    async def get(
        ...
        return await run_in_executor(None, _get, sprite_id, memory_type, ids, where, limit, offset, where_document, include)
    ...
    async def delete(
        ...
        await run_in_executor(None, _delete, sprite_id, memory_type, ids, where, where_document)
    ...
    async def get_by_highest_retrievability(
        self,
        sprite_id: str,
        memory_type: str,
        k: int = 5,
    ...
    async def similarity_search_by_vector_with_score_and_retrievability(
        self,
        embedding: list[float],
        sprite_id: str,
        memory_type: str,
    ...
    async def max_marginal_relevance_search_with_retrievability(
        self,
        query: str,
        sprite_id: str,
        memory_type: str,
        ...
        **kwargs: Any,
        ...
        return await self.max_marginal_relevance_search_by_vector_with_retrievability(
            embedding=embedding,
            sprite_id=sprite_id,
            memory_type=memory_type,
            k=k,
    ...
    async def max_marginal_relevance_search_by_vector_with_retrievability(
        self,
        sprite_id: str,
        memory_type: str,
        embedding: list[float],
