...
class AgentManager:
    ...
    虽然目前很多地方写的都是thread，但记住**agent = thread**

    因为以前一直都是写的thread，thread指langgraph checkpointer的thread。"""
    ...
    def __init__(self):
        """
        不要直接实例化此类。
        请使用 ThreadManager.create() 异步创建实例。
        """
        pass
    ...
    async def init_manager(self, heartbeat_interval: float = 5.0):
        ...
        self.thread_user_input_buffers = {}
        self.thread_gathereds = {}
        self.thread_streaming_tool_messages = {}
    ...
    async def trigger_thread(self, thread_id: str):
        ...
        # 更新记忆
        await self.process_timers(thread_id)
        ...
        if wakeup_call_time_seconds and not can_call:
            if current_agent_time_seconds >= wakeup_call_time_seconds:
                ...
                self_call_type = 'passive'
        if can_call:
            await self.call_agent('', thread_id, is_self_call=True, self_call_type=self_call_type)
        ...
        # 自动回收闲置上下文
            ...
            messages = await self.main_graph.get_messages(thread_id)
            ...
            if not isinstance(messages[-1], HumanMessage):
                ...
                if [m for m in not_extracted_messages if isinstance(m, HumanMessage) and not m.additional_kwargs.get("bh_from_system") and not m.additional_kwargs.get("bh_do_not_store")]:
                    ...
                    recycles = {t: recycle_memories(t, thread_id, not_extracted_messages, self.structured_model) for t in memory_types}
                    recycle_results = {}
                    if len(recycles) > 0:
                    ...
                    await self.main_graph.update_messages(thread_id, not_extracted_messages + remove_messages)

                    # 若有，将reflective的思考过程加入messages
                    if recycle_results.get('reflective'):
                        await self.main_graph.update_messages(thread_id, recycle_results.get('reflective', []))
    ...
    async def call_agent(
        ...
        if not is_self_call:
            ...
            if not self.thread_user_input_buffers.get(thread_id):
                self.thread_user_input_buffers[thread_id] = []
            if isinstance(user_input, list):
                self.thread_user_input_buffers[thread_id].extend(user_input)
            else:
                self.thread_user_input_buffers[thread_id].append(user_input)
        elif self.main_graph.thread_run_ids.get(thread_id):
            # 如果是自我调用同时已有运行，则不重复运行
            return

        # 生成运行id并加入运行id字典
        thread_run_id = str(uuid4())
        self.main_graph.thread_run_ids[thread_id] = thread_run_id

        # 当前线程有gathered，说明是在chatbot节点处打断了上次运行，则将上次运行结果加入中断数据字典
        if self.thread_gathereds.get(thread_id):
            interrupt_data = {}
            interrupt_data['chunk'] = self.thread_gathereds.pop(thread_id)
            if self.thread_streaming_tool_messages.get(thread_id):
                interrupt_data['called_tool_messages'] = self.thread_streaming_tool_messages.pop(thread_id)
            if interrupt_data:
                self.main_graph.thread_interrupt_datas[thread_id] = interrupt_data
        elif self.thread_streaming_tool_messages.get(thread_id):
            del self.thread_streaming_tool_messages[thread_id]
        ...
        if main_graph_state.next:
            ...
            while (
                ...
                if self.main_graph.thread_run_ids.get(thread_id, '') != thread_run_id:
                    return
                main_graph_state = await self.main_graph.graph.aget_state(config)
                current_node = main_graph_state.next[0]

        if not is_self_call:
            user_input = self.thread_user_input_buffers.pop(thread_id)
        else:
            if self.thread_user_input_buffers.get(thread_id):
                del self.thread_user_input_buffers[thread_id]
        ...
        if not is_self_call:
            ...
            if isinstance(user_input, str):
                ...
                if len(user_input) == 1:
                    ...
                    del self.main_graph.thread_run_ids[thread_id]
        ...
        async for typ, msg in self.main_graph.graph.astream(graph_input, config=config, context=context, stream_mode=["updates", "messages"]):
            if typ == "updates":
                ...
                if not self.main_graph.thread_run_ids.get(thread_id) or self.main_graph.thread_run_ids.get(thread_id) != thread_run_id:
                    canceled = True
                ...
                if not canceled and msg.get("chatbot"):
                    ...
                    streaming_tool_messages = []
                    last_message = ''
                    del self.thread_gathereds[thread_id]
                    del self.thread_streaming_tool_messages[thread_id]
                ...
                if isinstance(msg[0], AIMessageChunk):
                    ...
                    if first:
                        first = False
                        gathered = chunk
                        self.thread_gathereds[thread_id] = chunk
                        streaming_tool_messages = []
                        self.thread_streaming_tool_messages[thread_id] = []
                    else:
                        if chunk.id != gathered.id:
                            ...
                            gathered += chunk
                        self.thread_gathereds[thread_id] = gathered
                        ...
                        if 0 <= tool_index < len(tool_calls):
                            ...
                            if tool_calls[tool_index]['name'] == SEND_MESSAGE:
                                ...
                                if new_message:
                                    ...
                                    #await self.event_queue.put({"thread_id": thread_id, "name": "send_message", "args": {"content": new_message.replace(last_message, '', 1)}, "not_completed": True})
                                    ...
                                    if not chunk_completed:
                                        ...
                                        if tool_calls[tool_index].get('id'):
                                            ...
                                            self.thread_streaming_tool_messages[thread_id] = streaming_tool_messages
                                        last_message = ''
                            ...
                            if chunk_completed:
                                #if tool_calls[tool_index]['name'] == SEND_MESSAGE:
                                #    last_message = ''
                                #    await self.event_queue.put({"thread_id": thread_id, "name": "send_message", "args": {"content": ""}})
                                    #print('', flush=True)
                                if hasattr(self.main_graph.streaming_tools, tool_calls[tool_index]['name']):
                                    ...
                                    if tool_calls[tool_index].get('id'):
                                        ...
                                        self.thread_streaming_tool_messages[thread_id] = streaming_tool_messages
                                    await self.event_queue.put({"thread_id": thread_id, "name": tool_calls[tool_index]['name'], "args": tool_calls[tool_index]['args'], "id": tool_call_id})
                                    #print(await method(tool_calls[tool_index]['args']), flush=True)
                                tool_index += 1
                                loop_once = True


        if self.main_graph.thread_run_ids.get(thread_id) and self.main_graph.thread_run_ids.get(thread_id) == thread_run_id:
            del self.main_graph.thread_run_ids[thread_id]
        return

    async def process_timers(self, thread_id: str):
        await memory_manager.update_timers(thread_id)


    async def command_processing(self, thread_id: str, user_input: str):
        config = {"configurable": {"thread_id": thread_id}}
        message = '无效命令'
        if user_input == "/help":
            message = """可用指令列表：
    ...
    数量: 要删除的最后消息条数，例如 /delete_last_messages 3"""
                ...
                if message_count > 0:
                    _main_messages = await self.main_graph.get_messages(thread_id)
                    if _main_messages:
                        ...
                        await self.main_graph.update_messages(thread_id, remove_messages)
    ...
    唤醒agent（重置agent的自我调用状态），这可能导致agent对prompt有些误解。"""
                ...
                await self.main_graph.graph.aupdate_state(config, {"active_time_seconds": 0.0, "self_call_time_secondses": [], "wakeup_call_time_seconds": 0.0})
    ...
    查看agent消息列表中的所有消息。"""
                ...
                main_messages = await self.main_graph.get_messages(thread_id)
                if main_messages:
                ...
                if len(splited_input) >= 2 and splited_input[1]:
                    ...
                    if reset_type == 'config':
                        ...
                        if not main_state.next:
                            self.close_thread(thread_id)
                            await self.main_graph.graph.checkpointer.adelete_thread(thread_id)
                            ...
                            await self.init_thread(thread_id)
                            message = "已重置该线程所有数据。"
                        else:
    ...
    注意：在涉及现实时间的一些场景如网络搜索时agent可能会感到混乱"""
        ...
        await self.event_queue.put({"thread_id": thread_id, "name": "log", "args": {"message": message}, "id": 'command-' + str(uuid4())})
